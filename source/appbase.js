...
export class AppBase {
  ...
  onLoad(options) {
    ...
    if(options.u_member_id!=undefined){
      console.log("有没有往这里面走")
      AppBase.u_member_id=options.u_member_id;
    }
  ...
  onShow() {
    ...
    console.log(AppBase.u_member_id,'客户来了吗',AppBase.UserInfo.unionid)
      ...
      wx.login({
        success: res => {
          ...
          memberapi.getuserinfo({
            ...
            AppBase.UserInfo.u_member_id = AppBase.u_member_id;
            // AppBase.UserInfo.u_member_id = 2;
           memberapi.addmember(AppBase.UserInfo, (ret) => {
               console.log("走几次???",ret);
  ...
  getPhoneNo(e) {
    ...
    console.log(e.detail,'查看');
    api.decrypteddata(e.detail, (ret) => {
      console.log('收拾收拾',ret.return.phoneNumber);
      console.log(ret);
  ...
  copytext(e) {
    ...
    var instinfo = this.Base.getMyData().instinfo;
    wx.setClipboardData({ 
      data: '订单编号：'+text+'\r\n'+'公司名称：'+instinfo.company+'\r\n'+'开户银行：'+instinfo.bank+'\r\n'+'开户账号：'+instinfo.zhanghao,
      success: function (res) {
        wx.showToast({
          title: '复制成功',
        });
